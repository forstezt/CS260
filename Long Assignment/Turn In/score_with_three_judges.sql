create or replace
PROCEDURE SCORE_WITH_THREE_JUDGES 
...
BEGIN
  ...
  IF curr_score IS NULL THEN    
    --Stores the score, calculated by adding up the 3 raw scores and 
    --multiplying that sum by the difficulty of the dive
    SELECT (SUM(djs.raw_score) * dd.dive_dd) INTO score
